...
describe('region placement (US-WG-02)', () => {
	it('has 24 regions including Underdepths', () => {
		const world = getTestWorld();
		expect(world.regions.length).toBe(24);
		...
		expect(ids).toContain('gallowmere');
		expect(ids).toContain('underdepths');
	});

	it('23 surface regions are present on the tile grid', () => {
		const world = getTestWorld();

...
function makeNPC(x: number, y: number, char: string, color: string, name: string, dialogue: string[], gives?: { hp?: number; atk?: number }): NPC {
	return { pos: { x, y }, char, color, name, dialogue, dialogueIndex: 0, gives, given: false };
}
